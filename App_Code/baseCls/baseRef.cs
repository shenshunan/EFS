...
namespace Efsframe.cn.baseCls
{
  ...
  public class baseRef : System.Web.UI.Page
  {
    ...
    public string QryErrLogList(HttpRequest Request, HttpResponse Response)
    {
      ...
      {
          string strGradeID = Request["gradeid"];

          strGradeID = DicCache.getInstance().getCode("DIC_GRADE", strGradeID);
      ...
      // 添加订单
      public string OrdersAdd(HttpRequest Request, HttpResponse Response)
      {
        string strXml = Request["txtXML"];
        return Orders.Pipe.OrdersAdd(strXml);
      }


      public string OrdersList(HttpRequest Request, HttpResponse Response)
      {
        string strXml = Request["txtXML"];
        return Orders.Pipe.ordersList(strXml);
      }

      public string OrdersProList(HttpRequest Request, HttpResponse Response)
      {
        string strOrderID = Request["txtXML"];
        return Orders.Pipe.ordersProList(strOrderID);
      }


      public string OrdersDetail(HttpRequest Request, HttpResponse Response)
      {
        string strOrderID = Request["txtXML"];
        return Orders.Pipe.ordersDetail(strOrderID);
      }


      public string ordersDel(HttpRequest Request, HttpResponse Response)
      {
        string strOrderID = Request["txtXML"];
        return Orders.Pipe.ordersDel(strOrderID);
      }

      public string ExpExcel(HttpRequest Request, HttpResponse Response)
      {
          string savePath = Server.MapPath("../pipe/upload_files/");
          return Orders.Pipe.ExpExcel(savePath);
      }
  }
}
